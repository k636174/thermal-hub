...
プリンター設定の印字可能幅に合わせ、ナローサイズの長さ170mmで月間カレンダーを横長の白黒画像にしてESC/POS送信します。

## 週間スケジュール

1. ナビゲーションの「週間スケジュール」を開く。
2. 印刷したい週の日付を指定して「この週を表示」を押す。週は月曜日から始まります。
3. 各日の予定を入力し、土日祝日等の日付を反転するチェックを調整する（初期状態では土日が反転）。
4. 「予定を保存」で入力内容を保存する。または登録済みプリンターを選択し、「週間スケジュールを印字」を押して保存と印字を同時に行う。

期間ヘッダーを付けず、7日分の日付・英語曜日（Mon～Sun）・予定（各日6行まで）を月曜日から順にESC/POSテキストとして送信します。日付行の右側には日の区切り線を印字し、予定の各行には穴開け余白として全角スペース2文字を付けます。未入力行にも同じ余白を送って空行の省略を防ぎ、7日分を先頭150mm以内に配置します。実機補正として日曜欄後に3行を追加し、その後に20mmのカット余白を設けます。反転する日付は前後にも全角スペースを加えて反転領域を広げます。予定本文の `!!文字!!` は区切り記号を除き、囲まれた文字だけを反転印字します。
