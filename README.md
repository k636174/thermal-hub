...
期間ヘッダーを付けず、7日分の日付・英語曜日（Mon～Sun）・予定（各日6行まで）を月曜日から順にESC/POSテキストとして送信します。日付行の右側には日の区切り線を印字し、予定の各行には穴開け余白として全角スペース2文字を付けます。未入力行にも同じ余白を送って空行の省略を防ぎ、7日分を先頭150mm以内に配置します。実機補正として日曜欄後に3行を追加し、その後に20mmのカット余白を設けます。反転する日付は前後にも全角スペースを加えて反転領域を広げます。予定は印字時だけ使用し、サーバーには保存しません。
